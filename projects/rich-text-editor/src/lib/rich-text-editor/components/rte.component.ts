...
export class CdkRichTextEditorComponent {
  ...
  private _enterSuggestion = (
    ...
    if (selection && startedNode) {
      ...
      if (focusNode && focusNode instanceof Text && focusNode == startedNode) {
        ...
        const viewRef: EmbeddedViewRef<Node> = this.suggestionList[
          triggerIndex
        ].inputTemplate.createEmbeddedView({ value: item.value });
  ...
  insertComponent(componentName: Type<Component>) {
    ...
    if (selection && selection.anchorNode) {
      ...
      // TODO: createComponent does not need componentFactoryResolver
  ...
  removeComponent(componentName: Type<Component>) {
    let componentNode = this._findParentWithTag(
      this._getSelectedNode(),
      this._getSelectorName(componentName)
    );
  ...
  onMouseUp = (event: MouseEvent) => {
    setTimeout(() => {
      ...
      if (currentSelection && currentSelection?.toString() != '') {
        ...
        const toolbarRect =
          this.quickToolbar.nativeElement.getBoundingClientRect();
  ...
  onDrop = (event: DragEvent) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      // TODO: ADD <IMG> WITH PLACEHOLDER AND EMIT THE FILE. THE APP SHOULD UPLOAD AND SEND IN A FINAL LIVE SRC PATH.
  ...
  onPaste = (event: ClipboardEvent) => {
    ...
    if (fileList && fileList.length > 0) {
      ...
      for (let i = 0; i < fileList.length; i++) pasteFile(fileList.item(i));
    }
  };
