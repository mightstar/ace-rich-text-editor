...
export class CdkSuggestionComponent {
  @Output('cdkSuggestionSelected')
  select = new EventEmitter<CdkSuggestionSelect>();
  ...
  show = (visible: boolean) => {
    ...
    if (!visible) {
      this.container.nativeElement.classList.toggle('show', false);
      this.query = '';
    ...
    if (selection) {
      if (selection.isCollapsed && selection.rangeCount > 0) {
        ...
        if (editorRect) {
          ...
          this.container.nativeElement.classList.toggle('show', true);
        }
      }
    }
  };
  ...
  onValueChange = (event: Event) => {
    ...
    if (ev.data && (this.isVisible === false || (this.isVisible && this.filteredSuggestions.length == 0))) {
      const listIndex = this.suggestionList.findIndex(
        (item) => item.trigger == ev.data
      );
